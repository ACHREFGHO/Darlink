...
export function Step1Info({ data, updateData, onNext, mode = 'create', isSaving = false }: Step1Props) {
    ...
    // But since we are initing from props, we might need a useEffect if we want to persist previews 
    // For now, let's just handle new selections.
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                        ...
                        {/* Manual Select because shadcn Select is harder to integrate with raw react-hook-form without Controller */}
                        <select
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="governorate" {...register('governorate', { required: true })} placeholder="e.g. Nabeul" />
                    </div>

                    <div className="space-y-2">
                        <Label htmlFor="city">City</Label>
                        <Input id="city" {...register('city', { required: true })} placeholder="e.g. Hammamet" />
                    </div>

                    <div className="space-y-2">
                        <Label htmlFor="address">Address</Label>
                        <Input id="address" {...register('address', { required: true })} placeholder="Street address" />
                    </div>
                </div>
                    ...
                    {(previews.length > 0 || existingImagesObj.length > 0) && (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                            ...
                            {existingImagesObj.map((img) => (
                                <div key={img.id} className="relative aspect-video rounded-lg overflow-hidden border bg-gray-100 group">
                                    <img src={img.image_url} alt="Existing property photo" className="w-full h-full object-cover" />
                                    <div className="absolute top-1 left-1 bg-black/50 text-white text-[10px] px-2 rounded">Saved</div>
                                    <button
                                        type="button"
                            ...
                            {previews.map((src, index) => (
                                <div key={index} className="relative aspect-video rounded-lg overflow-hidden border bg-gray-100 group">
                                    ...
                                    <div className="absolute top-1 left-1 bg-green-500/80 text-white text-[10px] px-2 rounded">New</div>
                                    <button
                                        type="button"
